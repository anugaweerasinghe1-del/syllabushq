...
export const Route = createFileRoute("/suggest")({
  ...
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Help shape SyllabusHQ. Suggest features, subjects, or topics you want next.",
      },
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await submit({
        data: { name: name.trim(), idea: idea.trim(), visitorToken: getVisitorToken() },
      });
      if (res.ok) {
    ...
    <div className="min-h-screen">
      ...
      <main className="mx-auto max-w-3xl px-4 py-10 sm:px-6 sm:py-14">
        <header className="mb-10 rise">
          <p className="text-[10px] font-medium uppercase tracking-[0.22em] text-orange">Roadmap</p>
          <h1 className="mt-2 font-display text-5xl text-foreground sm:text-6xl">
            Suggest a feature
          </h1>
          <p className="mt-3 max-w-2xl text-muted-foreground">
            Missing a subject? Want past-paper timers, English/Sinhala/Tamil medium, study groups?
        ...
        <section className="glass-panel rounded-2xl p-5 sm:p-7">
          <form onSubmit={onSubmit} className="space-y-4">
            <input
              ...
              className="w-full rounded-lg border border-hairline bg-surface px-3 py-2.5 text-sm text-foreground outline-none focus:border-primary"
            />
            <textarea
              required
              minLength={4}
            ...
            <div className="flex items-center justify-end">
              <button
                type="submit"
                disabled={busy}
                className="inline-flex items-center justify-center rounded-lg bg-primary px-5 py-2.5 text-sm font-semibold text-primary-foreground transition hover:brightness-110 disabled:opacity-50"
              >
